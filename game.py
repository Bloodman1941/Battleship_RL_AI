...
class BattleshipGame:
    ...
    def get_remaining_ship_lengths(self, player_id: int):
        if player_id == 1:
            playerpieces = self.player_1_pieces
        else:
            playerpieces = self.player_2_pieces
        return [len(ship['tiles']) for ship in playerpieces.values() if not ship['is_sank']]
    
    def get_remaining_ship_count(self, player_id: int):
        if player_id == 1:
            playerpieces = self.player_1_pieces
        else:
            playerpieces = self.player_2_pieces
        return sum(1 for ship in playerpieces.values() if not ship['is_sank'])
    
    def is_game_over(self):
        return all(ship['is_sank'] for ship in self.player_1_pieces.values()) or all(ship['is_sank'] for ship in self.player_2_pieces.values())
    
    def get_winner(self):
        p1_dead = all(ship['is_sank'] for ship in self.player_1_pieces.values())
        p2_dead = all(ship['is_sank'] for ship in self.player_2_pieces.values())
    ...
    def reset_boards(self, seed1: int = 0, seed2: int = 0):
        self.player_1_pieces = self.get_game_pieces()
        self.player_2_pieces = self.get_game_pieces()
        self.player_1_board = self.get_clear_board()
        self.player_2_board = self.get_clear_board()
        self.set_random_board(player_id=1, seed=seed1)
        self.set_random_board(player_id=2, seed=seed2)