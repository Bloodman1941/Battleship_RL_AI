...
MoveFN = Callable[[list, list], Tuple[list, int, int]]
...
def run_headless_match(game: BattleshipGame, p1_move_fn: MoveFN, p2_move_fn: MoveFN, max_turns: int = 1000):
    turns = 0
    current_player = 1
    while not game.isgameover() and turns < max_turns:
        if current_player == 1:
            board = game.getviewboardopponent(opponentid=2)
            ships = game.getremainingshiplengths(1)
            _, x, y = p1_move_fn(board, ships)
            game.fireattile(opponentid=2, xposition=x, yposition=y)
            current_player = 2
        else:
            board = game.getviewboardopponent(opponentid=1)
            ships = game.getremainingshiplengths(2)
            _, x, y = p2_move_fn(board, ships)
            game.fireattile(opponentid=1, xposition=x, yposition=y)
            current_player = 1
        turns += 1
    return game.getwinner(), turns
    
def spd_move(board, ships):
    return ai.find_best_move(board, ships)

def main():
    if os.environ.get('BATTLESHIP_HEADLESS') == '1':
        game = BattleshipGame(size=10)
        game.set_random_board(player_id=1, seed=4)
        game.set_random_board(player_id=2, seed=1)
        winner, turns = run_headless_match(game, spd_move, spd_move)
